...
## Step 5: Run Simulation and Inference WITH beta drift (with and without prior beta drift)
...
beta_drift <- (max(true_beta_vec) - min(true_beta_vec)) / (T - 2) # zero for no prior drift 
...
## Step 7: Posterior Summary (with and without prior drift)
...
plot(beta_mean, type = "l", ylim = range(c(beta_lower, beta_upper)), 
     main = expression(paste("Posterior Mean and 95% CI of ", beta(t), " with drift (with(out) prior drift)")), 
...
# Autocorrelation for β(t) at t = 5, 15, 25
# Select time indices
time_points <- c(5, 15, 25)

# Plot autocorrelation for beta at selected time points
par(mfrow = c(length(time_points), 1))  # One plot per row

for (t in time_points) {
  beta_t <- samples$beta[, t]
  acf(na.omit(beta_t),
      main = paste("Autocorrelation of β(t) at t =", t),
      lag.max = 60)
}

# Plot autocorrelation for gamma at selected time points
par(mfrow = c(length(time_points), 1))  # One plot per row

for (t in time_points) {
  beta_t <- samples$gamma[, t]
  acf(na.omit(beta_t),
      main = paste("Autocorrelation of γ(t) at t =", t),
      lag.max = 60)
}

## Step 8: Predictive Checks (with prior drift)

# Plot of Observed vs Expected new claims over time (with prior drift) 
